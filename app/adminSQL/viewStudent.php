...
<body class="bg overflow-hidden">
    ...
    WHERE users.id = :userId AND users.role = 'student'";
    $stmt = $conn->prepare($query);
    $stmt->bindParam(":userId", $id);
    $stmt->execute();
    $dataUser = $stmt->fetch(PDO::FETCH_ASSOC);

    $query = "SELECT lessons.id AS lesson_id, lessons.lesson_name, exams.exam_score, exams.exam_date FROM lessons
        ...
        <div class="flex flex-col gap-4 text-center bg-myDark text-wheat p-4 rounded-lg">
            ...
            ?>
                <div class="flex flex-col border-solid border-wheat border-b-2 mb-4 p-2 gap-4">
                    ...
                    <div class="flex gap-2 font-bold w-full justify-between">
                        <div>Lesson Avarage</div>
                        <div><?= $lessonAverage[$id][$lessonId] ?></div>
                    </div>
                </div>
            <?php } ?>
            <a href="../admin/manageStudents.php" class="p-1 border-solid border-b-2 border-t-2 border-transparent rounded-full hover:border-wheat">Back</a>
        </div>
    </div>

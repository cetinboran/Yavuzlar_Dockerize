...
<body class="bg overflow-hidden">
    ...
    if (isset($_POST['sendedData'])) {
        ...
        if($password != $confirmPassword){
            header("Location: updateTeacher.php?sendedData=$id&error=3");
            ob_end_flush();
            return;
        }
        ...
        if(count($columns) == 1){
            header("Location: updateTeacher.php?sendedData=$id");
            ob_end_flush();
            return;
        }
        ...
        <form action="./updateTeacher.php" method="POST">
            <div class="flex items-center flex-col gap-4 bg-myDark p-8 text-center rounded-lg">
                ...
                <div class="text-wheat">
                    <button name="sendedData" value=<?= $_GET['sendedData'] ?> class="p-4 border-solid border-b-2 border-t-2 border-transparent rounded-full hover:border-wheat" type="submit">Update Teacher</button>
                    <a href="../admin/manageTeachers.php" class="p-4 border-solid border-b-2 border-t-2 border-transparent rounded-full hover:border-wheat" type="submit">Back</a>
                </div>
            </div>
        </form>
    </div>
